...
class scheduler_uam:
    ...
    def MCTS_simulation_step(self, cur_state, action, depth, cur_reward, discount = 1.0, rand=True):
        ...
        if depth == 0:
            ...
            return total_reward
        next_state, out_bool, time_val = self.estimate_MDP.scheduler_step(tuple(cur_state), action)
    ...
    def learning_phase_hard(self, task_index):
        ...
        while counter_a < self.num_samples or counter_c < self.num_samples:
            ...
            if current_state != tuple(["Terminal"]) and current_state != ["Terminal"] and current_state[task_index].is_same(
                ...
                time_count_c = 0
                counter_a += 1
            if current_state != tuple(["Terminal"]) and current_state != ["Terminal"] and finished_bool == True and (0 not in current_state[task_index].c_i.keys() or current_state[task_index].c_i[0] != 1.0):
                time_count = 0
                finished_bool = False
                time_count_c = 0
                time_count += (start_job.d_i - current_state[task_index].d_i)
            if current_state != tuple(["Terminal"]) and current_state != ["Terminal"] and (0 not in current_state[task_index].c_i.keys() or current_state[task_index].c_i[0] != 1.0) and current_state[task_index].d_i == 0:
                finished_bool = True
            action = task_index
            ...
            next_state, next_bool, time_val = self.MDP.scheduler_step(current_state, action)
            prev_state = current_state
            current_state = next_state
            if action == task_index and finished_bool == False:
                time_count_c += time_val
    ...
    def learning_phase(self, task_index):
        ...
        while counter_a < self.num_samples or counter_c < self.num_samples:
            ...
            if current_state != tuple(["Terminal"]) and current_state[task_index].is_same(start_job) and time_count != 0:
                ...
                time_count = 0
                counter_a += 1
            next_state, next_bool, time_val = self.MDP.scheduler_step(current_state, task_index)
            current_state = next_state
            time_count += time_val
    ...
    def make_estimate_MDP(self, depth):
        ...
        self.estimate_MDP = uam_mdp(new_task_list)
